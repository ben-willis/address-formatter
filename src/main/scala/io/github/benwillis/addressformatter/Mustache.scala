  ...
  val globals: Map[String, Any] = {
    ...
    Map(
      (this
        ...
        .map(x => {
          x.getName ->
            (if (x.getParameterTypes.length == 0) () => { x.invoke(this) } else
      ...
      state match {
        case Text =>
          if (cur == otag.charAt(0))
            if (otag.length > 1) { tagPosition = 1; state = OTag } else {
              staticText; state = Tag
            } else buf.append(cur)

        case OTag =>
          if (cur == otag.charAt(tagPosition))
            if (tagPosition == otag.length - 1) { staticText; state = Tag } else {
        ...
        case Tag =>
          if (buf.isEmpty && cur == '{') {
            ...
            if (ctag.length > 1) { tagPosition = 1; state = CTag } else tag
          } else buf.append(cur)
...
trait CompositeToken {
  def composite(tokens: List[Token],
                context: Any,
                partials: Map[String, Mustache],
                callstack: List[Any]): TokenProduct =
